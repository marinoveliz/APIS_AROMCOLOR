...
codeunit 50101 "TEC_CRMIntegrationMgt"
{
    ...
    // =========================================================================
    // PROCEDIMIENTOS HTTP Y JSON (ACTUALIZADOS A DINÁMICOS)
    // =========================================================================
    local procedure UpdateCRMOrderStatus(CRMOrderId: Guid; EstadoAEnviar: Text)
    var
        CRMSetup: Record "TEC_CRMIntegrationSetup";
        HttpClient: HttpClient;
        Content: HttpContent;
        Headers: HttpHeaders;
        Response: HttpResponseMessage;
    ...
    begin
        ...
        CRMSetup.Get();
        CRMSetup.TestField("Status Update URL");
        ...
        HttpClient.DefaultRequestHeaders.Add('Authorization', 'Bearer ' + GetAccessToken(CRMSetup));

        if HttpClient.Post(CRMSetup."Status Update URL", Content, Response) then begin
            if not Response.IsSuccessStatusCode() then begin
                Response.Content.ReadAs(PayloadText);
                Error('Error al actualizar el estado. Código: %1. Detalle: %2', Response.HttpStatusCode, PayloadText);
            end;
    ...
    // Pasamos el Setup como parámetro para no leer la BD dos veces
    local procedure GetAccessToken(CRMSetup: Record "TEC_CRMIntegrationSetup"): Text
    var
        HttpClient: HttpClient;
        Content: HttpContent;
        Headers: HttpHeaders;
        ...
        AuthURL: Text;
    begin
        CRMSetup.TestField("Tenant ID");
        CRMSetup.TestField("Client ID");
        CRMSetup.TestField("Client Secret");

        // Armamos el cuerpo de la petición con los campos de la tabla
        PostData := 'grant_type=' + CRMSetup."Grant Type" +
                    '&client_id=' + CRMSetup."Client ID" +
                    '&client_secret=' + CRMSetup."Client Secret" +
                    '&scope=' + CRMSetup."Scope";

        // Armamos la URL dinámica con el Tenant ID
        AuthURL := 'https://login.microsoftonline.com/' + CRMSetup."Tenant ID" + '/oauth2/v2.0/token';
        ...
        if HttpClient.Post(AuthURL, Content, Response) then begin
            Response.Content.ReadAs(ResponseText);
            if Response.IsSuccessStatusCode() then begin
                if JObject.ReadFrom(ResponseText) then
                    if JObject.Get('access_token', JToken) then
    ...
    begin
        // Traemos la configuración
        CRMSetup.Get();
        CRMSetup.TestField("Order Creation URL");

        AccessToken := GetAccessToken(CRMSetup);
        Content.WriteFrom(JsonPayload);
        Content.GetHeaders(Headers);
        if Headers.Contains('Content-Type') then Headers.Remove('Content-Type');
        Headers.Add('Content-Type', 'application/json');
        ...
        if HttpClient.Post(CRMSetup."Order Creation URL", Content, Response) then begin
            Response.Content.ReadAs(ResponseText);
